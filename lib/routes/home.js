...
route.get('/:ul' + utils.localePrefix + '?', function(req, res, next) {
	...
	recentPeriod.setHours(-48, 0, 0, 0);
	...
	function getStoriesFromDynamo(storiesIds) {
	...
	var props = {
		...
		activeStories: Data.webdata.access.stories({
				culture: currentCulture,
				limit: 4,
				order: '-countShares',
				where: {
	...
	Promise.props(props).then(function(result) {
		...
		res.locals.activeStories = result.activeStories;
		// console.log(result.activeStories[0]);
		res.locals.oneList = {
